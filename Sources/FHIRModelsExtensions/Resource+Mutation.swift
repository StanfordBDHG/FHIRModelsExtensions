...
extension FHIRType {
    ...
    public mutating func appendElements<C: RangeReplaceableCollection>(
        ...
        var collection = self[keyPath: keyPath] ?? C()
        collection.reserveCapacity(collection.count + elements.count)
        collection.append(contentsOf: elements)
        self[keyPath: keyPath] = collection
    }
    ...
    /// Removes all elements of the property that match the predicate.
    ///
    /// Also sets the property to `nil` if there are no elements remaining after the removal.
    ///
