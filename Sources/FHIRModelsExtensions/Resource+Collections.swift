...
public protocol FHIRTypeWithExtensions: FHIRType {
    var `extension`: [Extension]? { get set }
}

// Since FHIRModels 0.9, `Element` and `DomainResource` are protocols, which cannot be conformed to another protocol.
// The conformances are therefore declared per concrete type, in the generated `FHIRTypeWithExtensions+Conformances.swift`.
